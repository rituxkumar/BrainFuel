...
const CommentTableItem = ({ comment, fetchComments }) => {
 ...
 const  deleteComment = async () =>{
    try {
      const confirm = window.confirm('Are you sure you want to delete this comment ? 👁️')
       const {data} = await axios.post('/api/v1/delete-comment',{id:_id})
       if(data.success){
        toast.success(data.message)
        fetchComments()
       }else{
    ...
    <tr className="border-y border-gray-300">
      ...
      <td className="px-6 py-4">
        <div className="inline-flex items-center gap-4">
          {
          !comment.isApproved ? <img className="w-5 hover:scale-110 transition-all cursor-pointer" src={assets.tick_icon} onClick={approveComment} /> 
          :
